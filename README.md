...
| 模块 | 服务对象 | 当前状态 |
| --- | --- | --- |
| 童趣游戏乐园 | 儿童直接体验，家长陪伴 | 11 款学习游戏；童趣森林第一季五章已实现 |
| 小小寓言屋 | 家长操作，儿童收听与互动 | 25 篇固定故事与 AI 个性故事生成均已实现 |
...
- 小小整理家：分类、任务执行和检查习惯
- 图形建筑师：图形辨认、空间位置和组合
- 舒尔特方格：按顺序寻找 1～36、完成计时和个人速度历史

所有游戏都采用温和反馈、不扣分、不设置失败压力的设计原则，并适配手机、平板和电脑。需要朗读的童趣森林、小游戏和汉字小森林共用儿童友好普通话音色选择模块，优先设备上的自然女声；识字发音会进一步放慢。舒尔特方格保持纯数字界面，只在当前浏览器保存完整通关的日期时间和用时。
...
├── package.json、server/          # Node 服务、DeepSeek、账号与 MySQL 数据访问
...
- AI 故事：DeepSeek 官方 `deepseek-v4-flash`；`AI_STORY_GENERATION_ENABLED` 保留为紧急关闭开关
- 密钥：仅存放在不会提交 Git 的根目录 `.env`
- 访客统计：MySQL 主存储，数据库异常时自动降级为本地日志
- 故事存储：25 篇预设故事和 AI 生成结果统一保存在 `stories` 表；原始家庭事件不入库
- 识字内容：30 个汉字保存在 `literacy_characters` 表；登录后学习进度可保存到家庭默认儿童档案
- 账号体系：手机号验证码登录、加密登录身份、服务端会话、家庭成员、最小儿童档案和监护人同意记录已实现
- 家庭同步：首次登录不会自动上传；仅在成人明确勾选并确认后，合并当前设备的 AI 故事、收藏和识字进度
...
- AI 故事生成成功后写入 MySQL；未登录时关联浏览器随机设备编号，登录后同时归属当前家庭和默认儿童档案。
- 家长输入的原始事件、小名、偏好和氛围只用于本次 DeepSeek 生成，不写入本项目数据库。
- 未登录设备可以读取最近 12 篇故事；登录后还会读取家庭故事。清空记录只删除当前范围内可删除的 AI 故事。
- 紧急情况下可设置 `AI_STORY_GENERATION_ENABLED=false`，页面会禁用生成按钮，服务端接口也会拒绝生成。
