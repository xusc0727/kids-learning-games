...
项目总规划见 [`docs/项目总览.md`](docs/项目总览.md)，AI 故事产品方案见 [`docs/产品规划/AI寓言故事工坊.md`](docs/产品规划/AI寓言故事工坊.md)，识字产品方案见 [`docs/产品规划/汉字小森林.md`](docs/产品规划/汉字小森林.md)，完整数据库字典见 [`docs/数据库表结构.md`](docs/数据库表结构.md)。
...
- 识字内容：30 个汉字保存在 `literacy_characters` 表；当前设备学习进度保存在浏览器
...
- AI 故事生成成功后写入 MySQL，并关联浏览器生成的随机设备编号；不建立登录账号或儿童档案。
- 家长输入的原始事件、小名、偏好和氛围只用于本次 DeepSeek 生成，不写入数据库。
- 当前设备可以读取最近 12 篇故事；点击“清空记录”会同时删除数据库中的该设备 AI 故事。
- 收藏仍保存在当前浏览器。
...
- 支持点字普通话朗读、当前设备“我认识了”记录，以及看图、听音、词语三类五轮小游戏。
- 不登录、不上传学习进度、不采集儿童录音，也不做识字能力排名。
