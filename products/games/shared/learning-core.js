window.LearningCore={
  sound:true,
  speak(text){if(!this.sound)return;window.PlaymoriVoice.speak(text,{rate:.78,pitch:1.04})},
  ...
  toggle(...buttons){this.sound=!this.sound;if(!this.sound)window.PlaymoriVoice.cancel();buttons.forEach(b=>{b.textContent=this.sound?(b.classList.contains('circle')?'🔊':'🔊 声音开着'):(b.classList.contains('circle')?'🔇':'🔇 声音关了')})},
